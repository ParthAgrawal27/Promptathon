...
export default function VendorProfile() {
  ...
  const { scoredVendors, weights, activeEvents } = useRiskEngine();
    ...
    <div className="vendor-profile animate-fade-in">
      <div className="page-header">
        <div>
          ...
          <h1>{vendor.name}</h1>
          <p>{vendor.region} · {vendor.country} · {vendor.tier} · {vendor.category} · {vendor.contractValue}</p>
        </div>
      ...
      <div className="vp-layout">
        ...
        <div className="vp-right">
          {/* Raw Parameters */}
          <div className="card">
            <div className="card-header"><span className="card-title">Raw Parameters</span></div>
            <div className="vp-params">
              {Object.entries(vendor.params).map(([key, val]) => {
                const cfg = parameterConfig[key];
                const modified = vendor.modifiedParams[key];
                const changed = Math.abs(modified - val) > 0.1;
                return (
                  <div key={key} className="vp-param-row">
                    <span className="vp-param-label">{cfg?.icon} {cfg?.label}</span>
                    <span className="vp-param-val font-mono">
                      {typeof modified === 'number' ? (Number.isInteger(modified) ? modified : modified.toFixed(1)) : val}
                      {cfg?.unit && <small> {cfg.unit}</small>}
                    </span>
          ...
          <div className="card">
            ...
            <div className="vp-reasoning">
              ...
              {vendor.contributions.OnTime_Delivery?.normalized >= 50 && <div className="vp-reason warning">📦 Delivery reliability below expectations — {vendor.params.OnTime_Delivery}% on-time rate.</div>}
              {vendor.contributions.Financial_Stability?.normalized >= 50 && <div className="vp-reason warning">💰 Financial stability concern — score of {vendor.params.Financial_Stability}/100.</div>}
